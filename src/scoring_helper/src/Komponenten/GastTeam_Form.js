...
{/*Hier liegt das Problem, dass immer ein leerer am Ende angehangen wird. Klärungsbedarf!!*/}
      this.setState({
        form_gast_spieler:          [...this.state.form_gast_spieler, obj],
        form_gast_SpielerRNummer:   '',
        form_gast_SpielerNName:     '',
        form_gast_SpielerVName:     '',
        form_gast_SpielerPosition:  '',
    ...
    }else if(this.submitInput.value =='Bearbeiten'){
    ...
    <div className="SpielInfo_Popup_inner">
  		...
  		<form className="form-horizontal" role="form">
             ...
             <input required type="Number"
                    ref={(ref) => this.gast_SpielerRNummer = ref}
                    onChange={event => this.setState({form_gast_SpielerRNummer: event.target.value})}
                    value={this.state.form_gast_SpielerRNummer}
                    className={this.state.perro}/>
  			</div>
  		  <div className="input-group">
             <span className="input-group-addon">Nachname</span>
             <input type="text"
                    required ref={(ref) => this.gast_SpielerNName = ref}
                    onChange={event => this.setState({form_gast_SpielerNName: event.target.value})}
                    value={this.state.form_gast_SpielerNName}
                    className="form-control"  />
  			</div>
  			<div className="input-group">
             <span className="input-group-addon">Vorname</span>
             <input type="text"
                    required ref={(ref) => this.gast_SpielerVName = ref}
                    onChange={event => this.setState({form_gast_SpielerVName: event.target.value})}
                    value={this.state.form_gast_SpielerVName}
                    className="form-control"/>
  			</div>
        ...
        <div className="input-group">
             ...
             <input type="Number"
                    required ref={(ref) => this.gast_SpielerPosition = ref}
                    onChange={event => this.setState({form_gast_SpielerPosition: event.target.value})}
                    value={this.state.form_gast_SpielerPosition}
                    className="form-control"/>
  			</div>
        ...
        <div className="input-group">
             ...
             <input type="Number"
                    required ref={(ref) => this.gast_SpielerPNummer = ref}
                    onChange={event => this.setState({form_gast_SpielerPNummer: event.target.value})}
                    value={this.state.form_gast_SpielerPNummer}
                    className="form-control"/>
        </div>
             ...
             <tbody>
                ...
                return
                  <Row
                      ...
                      users = {this.state.form_gast_spieler}
                      data = {data}
                      key={index}
                      row={index}
  ...
  class Row extends Component{
  	...
  	render(){
          ...
          <td>{this.props.data.form_gast_SpielerPNummer}</td>
  				...
  				<td className="glyphicon glyphicon-edit"
            onClick={() => {this.props.editRow(this.props.row)}}>
            <button>Bearbeiten</button>
          </td>

          <td className="glyphicon glyphicon-trash"
            onClick={() => {this.props.deleteRow(this.props.row)}}>
